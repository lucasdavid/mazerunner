...
class QLearning(object):
    ...
    def __init__(self, num_states=1024, num_action=4,
                 ...
                 action_selection=ACTION_SELECTION.greedy,
                 action_selection_epsilon=0.5):
        ...
        self._action = ACTIONS.forward
        self.distance_to_goal = np.inf
        self.state_binary = self.discretize(5 * [np.inf])
        self.state_int = int(self.state_binary, 2)
    ...
    def action(self):
        ...
        if (self._action_selection == ACTION_SELECTION.greedy):
            ...
            if self.random_state.rand() <= self._action_selection_epsilon:
                self._action = self.random_state.randint(0, self._num_action)
                print('random action = ', self._action)
            else:
                self._action = np.argmax(self.Q[self.state_int])
    ...
    def set_state(self, percept):
        ...
        """
        r = self.reward(self.state_binary, self._action,
        ...
        print('current_action=', self._action, ', tabelaQold=',
              self.Q[self.state_int])
        print('current state = ', self.state_binary)
        print(
            'current_action=', self._action, ', tabelaQnew=',
            self.Q[new_state_int])
        print('new state = ', new_state_binary)
    ...
    def discretize(self, p):
        ...
        """
        s = ''
        delta_distance = self.distance_to_goal - p[0]
        s = s + aux_state_discretization(delta_distance, (-1) * self._epsilon,
                                         self._epsilon)  # s=xx -> 00-aproximou, 01-manteve, 10-recuou
        s = s + aux_state_discretization(p[1], self._front_sonar_min_value,
                                         self._front_sonar_max_value)  # s=xxyy  -> acrescimo: sonar da frente --> 00-longe, 01-perto, 10-colisao
        s = s + aux_state_discretization(p[2], self._front_sonar_min_value,
                                         self._front_sonar_max_value)  # s=xxyyzz  -> acrescimo: sonar de tras
        s = s + aux_state_discretization(p[3], self._side_sonar_min_value,
                                         self._side_sonar_max_value)  # s=xxyyzzdd  -> acrescimo: sonar direita
        s = s + aux_state_discretization(p[4], self._side_sonar_min_value,
                                         self._side_sonar_max_value)  # s=xxyyzzddee  -> acrescimo: sonar esquerda
    ...
    def reward(self, state, action, distance_to_goal):
        ...
        while (index < size_list):
            t_state = self.get_translated_state(state_list[index],
            ...
            if (t_state == AGENT_STATES.closetogoal):
                ...
                total_reward += (self.IMMEDIATE_REWARD['closetothegoal'] *
                                 reward_weight)
            index = index + 1

        # rewards related to actions
        total_reward += self.IMMEDIATE_REWARD[action]
